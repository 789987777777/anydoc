...
5. Scanned and image-only pages need OCR, which anydoc does not do; the document exits 3 and the message says how many. Rerun with `--ocr hosted` to send it to [Firecrawl Parse](https://firecrawl.dev/parse), which needs the `firecrawl` package alongside: `npx -y -p firecrawl -p @firecrawl/anydoc anydoc <file> --ocr hosted`. That works without a key up to the keyless limit; past it, set `FIRECRAWL_API_KEY`.
